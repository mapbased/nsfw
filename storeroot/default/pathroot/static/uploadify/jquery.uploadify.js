				...
				if (args[0]) {
					...
					if (args[0] == '*') {
                        ...
                        for (var n = 0; n < args.length; n++) {
                            ...
                            $('#' + args[n]).find('.cancel').remove();// Add By Vernon 这里可能点击N次, 需要禁止掉cancel
                            $('#' + args[n]).find('.uploadify-progress-bar').remove();
                            $('#' + args[n]).delay(1000 + 100 * n).fadeOut(500, function () {
                                $(this).remove();
                            });
                            // ------------------ Add By Vernon ------------------
                            //console.log('====uploadLimit== ' +  settings.uploadLimit);
                            var newUploadLimit = settings.uploadLimit + 1;
                            swfuploadify.setFileUploadLimit(newUploadLimit);
                            settings.uploadLimit = newUploadLimit;
                            //console.log('====newUploadLimit== ' +  newUploadLimit);
                        }
					}
				} else {
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += '选中的图片数目不能大于规定的最多上传数量 ' + settings.queueSizeLimit + ' 张';
						}
						break;
